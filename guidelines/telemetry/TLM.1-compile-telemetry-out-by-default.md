...
- **True elision.** Tracy (`TRACY_ENABLE` undefined), Unity
  ...
  the Vigil `VIGIL_ENABLE_TRACE=OFF` build profile. The macro is
...
rather than a *verification step*. A `#define VIGIL_TRACE(...)
...
- **Pair the gate with build-profile fail-closed.** The build
  ...
  performance contract (e.g., `--performance --trace`); Vigil
  packet 142's exit-code-2 pattern is the right shape.
...
#if defined(VIGIL_PROFILE_ENABLED)
    #include <tracy/Tracy.hpp>
    #define VIGIL_ZONE(name)        ZoneScopedN(name)
    #define VIGIL_PLOT(name, value) TracyPlot(name, value)
#else
    #define VIGIL_ZONE(name)        ((void)0)
    #define VIGIL_PLOT(name, value) ((void)0)
#endif

void decode_loop(std::span<const Token> input) noexcept {
    VIGIL_ZONE("decode.loop");                 // disappears in release
...
void on_policy_change(int new_policy) noexcept {
    STAP_PROBE1(vigil, mtp_policy_changed, new_policy);
    // ... apply policy ...
}
...
- **NDEBUG is not the right gate.** `NDEBUG` controls `assert`;
  ...
  (`VIGIL_PROFILE_ENABLED`, `TRACY_ENABLE`, `OPTICK_ENABLE`).
...
- Vigil packet 142, *Unreal-Style Observability Boundary* —
  the compile-gate inventory and build-profile fail-closed
  pattern.

...
1. **P0 — The “good” WebGPU buffer planner dereferences an empty vector.**  
   [WASM.10:104](guidelines/wasm/WASM.10-request-the-capability-you-need.md:104)
...
2. **P1 — `emscripten_set_main_loop` return behavior is stated incorrectly.**  
   [WASM.3:45](guidelines/wasm/WASM.3-return-the-frame-to-the-event-loop.md:45)
...
3. **P1 — Wrong MIME type does not cause native `instantiateStreaming` to “silently fall back.”**  
   [WASM.7:36](guidelines/wasm/WASM.7-budget-startup-and-keep-the-code-cache.md:36), [WASM.7:145](guidelines/wasm/WASM.7-budget-startup-and-keep-the-code-cache.md:145)
...
4. **P1 — Historical V8 caching and compilation behavior is presented as current design guidance.**  
   [WASM.7:22](guidelines/wasm/WASM.7-budget-startup-and-keep-the-code-cache.md:22), [WASM.7:29](guidelines/wasm/WASM.7-budget-startup-and-keep-the-code-cache.md:29), [WASM.13:41](guidelines/wasm/WASM.13-ship-a-build-per-feature-set.md:41), [WASM.14:16](guidelines/wasm/WASM.14-state-the-target-matrix.md:16)
...
5. **P1 — WASM.1 makes non-universal memory-growth behavior categorical.**  
   [WASM.1:6](guidelines/wasm/WASM.1-size-linear-memory-and-resist-growth.md:6), [WASM.1:12](guidelines/wasm/WASM.1-size-linear-memory-and-resist-growth.md:12)
...
6. **P1 — The 128 MB-mobile/4 GB-desktop “32× range” is not established by the cited sources.**  
   [WASM.10:30](guidelines/wasm/WASM.10-request-the-capability-you-need.md:30), [WASM.14:28](guidelines/wasm/WASM.14-state-the-target-matrix.md:28)
...
7. **P1 — The native-build guard prevents the promised cross-target core build.**  
   [WASM.12:112](guidelines/wasm/WASM.12-keep-the-compute-core-natively-buildable.md:112)
...
8. **P1 — WASM.9 attributes the opposite memory behavior to Unity.**  
   [WASM.9:26](guidelines/wasm/WASM.9-keep-assets-compressed-to-their-destination.md:26)
...
9. **P1 — WASM.4 turns measured engine behavior and possible compiler lowering into universal facts.**  
   [WASM.4:15](guidelines/wasm/WASM.4-reduce-indirect-dispatch.md:15), [WASM.4:39](guidelines/wasm/WASM.4-reduce-indirect-dispatch.md:39), [WASM.4:97](guidelines/wasm/WASM.4-reduce-indirect-dispatch.md:97)
...
10. **P2 — Timer resolution is generalized from Chrome to “the browser.”**  
    [WASM.11:6](guidelines/wasm/WASM.11-state-the-measurement-conditions.md:6)
...
11. **P2 — Disabling autovectorization is presented as necessary for handwritten SIMD.**  
    [WASM.6:49](guidelines/wasm/WASM.6-target-128-bit-wasm-simd.md:49)
...
The packet’s literal “all prose is original” guarantee is nevertheless not quite true: [WASM.3:28](guidelines/wasm/WASM.3-return-the-frame-to-the-event-loop.md:28) reproduces Emscripten’s short phrase “something like 50% or so” as a quotation. This is attributed, brief, and not a material licensing exposure, but under the repository’s unusually strict no-copy rule it should be paraphrased.
...
- **P1:** [WASM.3:83](guidelines/wasm/WASM.3-return-the-frame-to-the-event-loop.md:83) transfers owning state through `void*`, allocates with explicit `new`, and destroys it via `delete &ctx`. That conflicts with `R.11`, `I.11`, and the non-owning interpretation of raw pointers in `R.3`. Use an owning registry/RAII handle with an explicitly non-owning callback pointer.
- **P2:** [WASM.1:76](guidelines/wasm/WASM.1-size-linear-memory-and-resist-growth.md:76) uses `malloc/free` despite `R.10`, and its alignment arithmetic lacks a nonzero-power-of-two precondition and overflow protection (`I.5`, `ES.103`).
- **P2:** [WASM.7:112](guidelines/wasm/WASM.7-budget-startup-and-keep-the-code-cache.md:112) indexes an empty vector when `iterations == 0`, violating `SL.con.3`; [WASM.11:92](guidelines/wasm/WASM.11-state-the-measurement-conditions.md:92) likewise lacks a nonzero-iterations precondition.
